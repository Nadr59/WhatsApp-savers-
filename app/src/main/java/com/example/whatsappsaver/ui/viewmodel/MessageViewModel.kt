...
class MessageViewModel @Inject constructor(private val repo: MessageRepository) : ViewModel() {
    ...
    val filteredMessages = combine(allMessages, _query, _cat) { messages, q, c ->
        var result = messages
        if (!c.isNullOrBlank() && c != "الكل") {
            result = result.filter { it.category == c }
        }
        if (q.isNotBlank()) {
            result = result.filter { it.messageText.contains(q, ignoreCase = true) }
        }
        result
    }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())
    ...
    fun insertMessage(m: Message) {
        viewModelScope.launch { repo.insertMessage(m) }
    }

    fun deleteMessage(m: Message) {
        viewModelScope.launch { repo.deleteMessage(m) }
    }

    fun togglePin(m: Message) {
        viewModelScope.launch { repo.togglePinStatus(m) }
    }
